...
-- 표현식에 특수문자 사용하기(문자열 안에 홀따옴표를 사용할 때)
select empno, '의 번호를 가진 사람 '' ', ename from emp;
select empno, q'[번호를가진사람'''']', ename from emp; -- 오라클 문자열 사용법(다른DB에서는 불가능할 수도 있다.)
...
select ename || '''s sal is ' || sal as "사원의 급여" from emp;
...
-- 1. emp 테이블에서 부서 번호가 30 이고, 직업이 salesman인 데이터를 조회
select *
from emp
where deptno = 30
and job = 'SALESMAN';
-- 2. emp 테이블에서 부서번호가 30 이거나, 직업이 salesman 인 데이터를 조회하기
select *
from emp
where deptno = 30
or job = 'SALESMAN';
-- 3. emp 테이블에서 hiredate 가 80/12/17 일 데이터를 조회
select *
from emp
where hiredate = '80/12/17';

-- where 절에서 사용하는 연산자 ( 산술 연산자 + - * / ) 
-- emp 테이블에서 sal * 12 = 36000 이 되는 sal을 가진 데이터를 출력
select *
from emp
where sal * 12 = 36000;

-- where 절에서 사용하는 연산자 ( 비교 연산자 >, >= , < , <= ) 
-- emp 테이블에서 sal 이 3000 이상인 데이터를 출력
select *
from emp 
where sal >= 3000;
...
select *
from emp
where ename > 'F'; -- FORD 가 F 보다 크다.

-- where 절에서 사용하는 연산자 ( 비교 연산자 = , != , <> )
-- emp 테이블에서 sla 이 3000 인 데이터를 조회
select *
from emp
where sal = 3000;
-- emp 테이블에서 sal 이 3000이 아닌 데이터를 조회
select *
from emp
where sal != 3000;
select *
...
-- IN 연산자 ( OR 연산자 대신 사용가능 )
-- emp 테이블에서 job(직업)이 MANAGER, SALESMAN, CLERK 인 데이터를 조회
-- 1. or 연산자 사용했을 때
select * from emp
where job = 'MANAGER'
or job = 'SALESMAN'
...
-- BETWEEN a AND b 연산자 ( a 이상 와 b 이하의 데이터를 조회 )
-- emp 테이블에서 sal(급여가) 2000 이상 3000 이하인 데이터를 조회
-- 1. between을 사용 안하고 작성
select * from emp
...
-- LIKE 연산자 ( 특정 패턴을 가지고 있는 데이터를 조회 )
-- % : 글자 수 제한 없이 어떠한 글자가 와도 상관이 없다.
-- _ : 글자 수 한 글자만 올 수 있고 어떠한 글자가 와도 상관이 없다.
-- emp 테이블에서 두번째 문자로 L을 가지는 직원의 이름을 출력하여라
select ename
from emp 
where ename like '_L%';
-- emp 테이블에서 S 로 시작하는 사원의 이름을 출력하여라
select ename
from emp
where ename like 'S%';
-- emp 테이블에서 AM 을 포함하는 사원의 이름을 출력하여라
select ename
from emp
where ename like'%AM%';
-- like 의 부정 ( not like )
...
-- 그래서, NULL 과 관련된 모든 연산은 FALSE가 나온다.
...
-- AND 연산과 OR 연산 차이점
select * from emp
where sal > null and comm is null; -- 모든 결과에 대해서 1번 조건이 false 라서 출력되는 조건 없음
select * from emp
where sal > null or comm is null; 
-- 모든 결과에 대해서 1번 조건이 false 지만, 2번 조건이 true가 되는 게 있기 때문에 출력값이 있다.

-- 쿼리 조회문을 정렬하기 ( ORDER BY 절 )
-- ORDER BY 조회기준이되는컬럼 asc(오름차순)/desc(내림차순)
...
-- INTERSECT : 교집합
-- MINUS : 차집합
-- 집합연산자는 2개의 쿼리문으로 나오는 결과값을 중심으로 집합을 연산한다.
...
-- 1. EMP 테이블에서 JOB(직업)을 중복 없이 출력하여라
select distinct job
from emp;
select job
from emp
...
select empno as "empoyee_no", ename as "empoyee_name", job, mgr as "manager", 
hiredate, sal as "salary", comm, deptno
from emp
order by deptno desc, ename asc;

-- 3. EMP 테이블에서 사원이름이 S 로 끝나는 사원 데이터를 모두 출력
select *
from emp
where ename like '%S';
-- 4. EMP 테이블에서 30번 부서에 근무하는 사원중에 직책이 SALESMAN인 사원의
...
where deptno =30
and sal > 2000;
-- 6. EMP 테이블에서 급여 값이 2000 이상 3000이하 범위 외의 값을 가진 사원의 이름, 급여를 조회하라
select ename, sal
from emp
...
--     급여가 1000이상 2000 이하가 아닌 사원의 이름(ename), 번호(empno), 급여(sal), 부서번호(deptno)를 출력
...
-- 8. 추가 수당(COMM)이 존재하지 않고, 상급자(mgr)가 존재하며, 직책(job)이 manager, clerk 인 사원 중
--     사원 이름의 두 번째 글자가 L 이 아닌 사원의 모든 정보를 출력하여라
select *
from emp 
where comm is null
and mgr is not null 
