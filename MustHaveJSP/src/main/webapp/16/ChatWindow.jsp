...
//event : webSocket.send(문자열)가 실행될때 매개변수로 사용된 데이터
webSocket.onmessage = function (event) {
	...
	// 메시지가 있는지 없는지 확인하는 if문
	if (content !== "") {
		// 귓속말 처리 코드
		// /(슬러시)가 있는지 확인하는 if문
		if (content.match("/")) {
			// 메시지 안에 있는 대화명이 자신의 이름이면 실행
...
<style>
#chatWindow{
	border: 1px solid black; width:270px; height:310px; overflow:scroll;
	padding:5px;
}
#chatMessage{width:236px; height:30px;}
#sendBtn{height:30px; position:relative; top:2px; left:-2px;}
#chatId{
	width:158px; height:24px; border:1px solid #AAAAAA; 
